		...
		<Box sx={styles.Wrapper}>
			<Box sx={styles.InnerWrapper(pokemonData.types[0].type.name)} >
				<Box sx={styles.Content}>
					<Typography sx={styles.Order}>#{`${pokemonData.order}`.padStart(3, `0`)}</Typography>
					<Box sx={styles.MainInfo}>
						...
						<Box sx={styles.TypeWrapper}>
							{pokemonData.types.map(singleType =>
								<Box sx={styles.TypeName(singleType.type.name)} >{singleType.type.name}</Box>
							)}
						</Box>
					</Box>
				</Box >
				<Box sx={styles.DetailsWrapper}>
					{pokemonData.base_experience &&
						<Box sx={styles.SmallCardWrapper}>
							...
							<Typography sx={styles.Data}>{pokemonData.base_experience}</Typography>
						</Box>
					}
					{pokemonData.height &&
						<Box sx={styles.SmallCardWrapper}>
					...
					{pokemonData.stats.map(singleStats => {
						return (
							<Box sx={styles.SmallCardWrapper} key={singleStats.stat.name}>
								<Typography sx={styles.Title}>{singleStats.stat.name}</Typography>
								<Typography sx={styles.Data}>{singleStats.base_stat}</Typography>
							</Box>
						)
					})
					}
